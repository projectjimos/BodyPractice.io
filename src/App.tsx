  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 flex flex-col gap-6">
        <div className={`rounded-2xl p-6 border border-slate-800 bg-gradient-to-r ${activeOrgan.bgGradient} relative overflow-hidden shadow-2xl`}>
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <h2 className="text-3xl font-black text-white tracking-tight">{activeOrgan.title}</h2>
              <p className="text-sm text-slate-300 mt-1 max-w-2xl font-medium">{activeOrgan.description}</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="bg-slate-900/80 hover:bg-slate-800 border border-slate-700 text-slate-200 px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
              >
                <Trophy className="w-3.5 h-3.5 text-purple-400" /> Take 100-Question Quiz
              </button>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'explorer' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            <div className="lg:col-span-7 flex flex-col gap-4">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl flex flex-col items-center">
                <div className="w-full flex items-center justify-between mb-3 border-b border-slate-800/80 pb-2">
                  ...
                  <span className="text-[11px] text-slate-500">Click hotspots to inspect structures</span>
                </div>

                {activeOrganId === 'eye' && <EyeDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'brain' && <BrainDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'heart' && <HeartDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'lungs' && <LungsDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'skeleton' && <SkeletonDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'digestive' && <DigestiveDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
                {activeOrganId === 'ear' && <EarDiagram parts={activeOrgan.parts} selectedPartId={selectedPartId} onSelectPart={setSelectedPartId} />}
              </div>
            ...
            <div className="lg:col-span-5">
              {selectedPart ? (
                <PartDetailPanel part={selectedPart} onClose={() => setSelectedPartId(null)} />
              ) : (
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center text-slate-400 flex flex-col items-center justify-center min-h-[300px]">
                  ...
                  <p className="text-sm font-semibold">Select any part from the diagram or list to inspect its anatomy and pronunciation.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'lab' && <InteractiveLab organSystem={activeOrgan} onSelectPart={setSelectedPartId} selectedPartId={selectedPartId} />}
        {activeTab === 'quiz' && <QuizSection organSystem={activeOrgan} />}
      </main>
